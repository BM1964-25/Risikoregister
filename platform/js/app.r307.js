import { createStore, cloneState } from "./state.r307.js";
import { modules, riskCategoryOptions, normalizeRiskCategoryValue, calculatePertResult, calculateMonteCarloResult, calculateEarnedValueResult, buildManagementReportData, buildSelectedReportData, renderSelectedReportText, renderRiskReportText } from "./modules.r307.js";
...
function applyAiWorkshopFreeTextToRisk() {
  ...
  store.setState((current) => ({
    ...
    riskRegister: {
      ...
      risks: current.riskRegister.risks.map((risk) => {
        if (risk.id !== targetRiskId) return risk;
        const nextLikelihood = Math.max(1, Math.min(5, Number(first.likelihood) || Math.ceil((Number(first.probabilityPercent) || 0) / 20) || risk.likelihood || 3));
        ...
        const nextProbabilityPercent = Math.max(0, Math.min(100, Number(first.probabilityPercent) || nextLikelihood * 20));
        return {
          ...risk,
