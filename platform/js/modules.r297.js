      ...
      return `
        <div class="module-shell risk-register-shell">
          ...
          <details class="info-card risk-fold-card ai-workshop-card" open style="margin-top:18px;">
            ...
            <p class="form-note">Für Freitext erzeugst du neue Risiken. Für Maßnahmenvorschläge wählst du oben ein Zielrisiko. Der Übernehmen-Button passt sich daran an.</p>
            <div class="ai-workshop-result-actions" style="margin-top:12px;">
              <button class="action-btn primary" type="button" data-action="apply-ai-workshop-result" ${state.ui?.aiWorkshop?.activeTask === "measures-residual" && !String(state.ui?.aiWorkshop?.selectedRiskId || "").trim() ? "disabled" : ""}>${state.ui?.aiWorkshop?.activeTask === "measures-residual" ? "Bestehendes Risiko aktualisieren" : "Neues Risiko anlegen"}</button>
