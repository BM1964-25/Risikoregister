...
function bindEvents() {
  ...
  document.addEventListener("dragover", (event) => {
    const panel = event.target?.closest?.("[data-risk-panel-key]");
    if (!panel || !draggedRiskPanelKey) return;
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target?.dataset?.action === "close-risk-register-panels") {
      store.setState((state) => ({
        ...state,
        ui: {
          ...state.ui,
