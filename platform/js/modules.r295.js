        ...
        <div class="module-shell risk-register-shell">
          ...
          <details class="info-card risk-fold-card ai-workshop-card" open style="margin-top:18px;">
            ...
            <div class="ai-workshop-result-actions" style="margin-top:12px;">
              <button class="action-btn primary" type="button" data-action="apply-ai-workshop-result" ${state.ui?.aiWorkshop?.activeTask === "measures-residual" && !String(state.ui?.aiWorkshop?.selectedRiskId || "").trim() ? "disabled" : ""}>${state.ui?.aiWorkshop?.activeTask === "measures-residual" ? (String(state.ui?.aiWorkshop?.selectedRiskId || "").trim() ? "Bestehendes Risiko aktualisieren" : "Zielrisiko wählen") : "Neues Risiko anlegen"}</button>
