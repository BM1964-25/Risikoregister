...
function parseMonteCarloNumber(value) {
...
function normalizeMonteCarloKey(value, index = 0) {
  ...
  return /^\d/.test(safe) ? "v_" + safe : safe;
}

function sampleMonteCarloNormal(mean, stdDev, rng) {
...
function sampleMonteCarloTriangular(min, mode, max, rng) {
  const a = Number(min) || 0;
  const b = Number(mode) || a;
  const c = Number(max) || b;
  if (c <= a) return a;
  const pivot = (b - a) / (c - a);
  const u = rng();
  if (u < pivot) {
...
function sampleMonteCarloPert(min, mode, max, rng) {
  const a = Number(min) || 0;
  const b = Number(mode) || a;
  const c = Number(max) || b;
  if (c <= a) return a;
  const range = c - a;
  const alpha = 1 + 4 * ((b - a) / range);
  const beta = 1 + 4 * ((c - b) / range);
  const sampleGamma = (shape) => {
    ...
    const d = shape - 1 / 3;
    const c1 = 1 / Math.sqrt(9 * d);
    while (true) {
      let x;
      let v;
      do {
        x = sampleMonteCarloNormal(0, 1, rng);
        v = 1 + c1 * x;
      } while (v <= 0);
      v = v * v * v;
      const u = rng();
  ...
  const betaSample = x / Math.max(x + y, Number.EPSILON);
  return a + betaSample * range;
}

function sampleMonteCarloLogNormal(mean, stdDev, rng) {
  const mu = Number(mean) || 0;
  const sigma = Math.max(0, Number(stdDev) || 0.2);
  const center = Math.log(Math.max(mu, 1));
  return Math.exp(sampleMonteCarloNormal(center, sigma, rng));
}

function sampleMonteCarloDistribution(variable, rng) {
  const distribution = String(variable.distribution || "uniform").toLowerCase();
  const min = Number(variable.minimum ?? variable.min ?? 0) || 0;
  const likely = Number(variable.likely ?? variable.mode ?? variable.mean ?? min) || min;
  const max = Number(variable.maximum ?? variable.max ?? likely) || likely;
  const mean = Number(variable.mean ?? likely) || likely;
  const stdDev = Number(variable.stdDev ?? variable.standardDeviation ?? 0) || 0;

  switch (distribution) {
    case "normal":
      return Math.max(0, sampleMonteCarloNormal(mean, stdDev || Math.max(Math.abs(max - min) / 6, 1), rng));
    case "lognormal":
      return Math.max(0, sampleMonteCarloLogNormal(mean || likely, stdDev || 0.25, rng));
    case "pert":
    case "beta-pert":
      return sampleMonteCarloPert(min, likely, max, rng);
    case "triangular":
    case "dreieck":
      return sampleMonteCarloTriangular(min, likely, max, rng);
    case "linear":
    case "uniform":
    default:
      return min + rng() * Math.max(max - min, 0);
  }
}

function normalizeMonteCarloVariable(item, index = 0) {
  const key = normalizeMonteCarloKey(item?.key || item?.name || ("var_" + (index + 1)), index);
  return {
    id: item?.id || "mc-var-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7),
    key,
    name: String(item?.name || "Variable " + (index + 1)).trim(),
    description: String(item?.description || "").trim(),
    unit: String(item?.unit || "").trim(),
    distribution: String(item?.distribution || "uniform").toLowerCase(),
    minimum: parseMonteCarloNumber(item?.minimum ?? item?.min),
    likely: parseMonteCarloNumber(item?.likely ?? item?.mode ?? item?.mostLikely),
    maximum: parseMonteCarloNumber(item?.maximum ?? item?.max),
    mean: parseMonteCarloNumber(item?.mean),
    stdDev: parseMonteCarloNumber(item?.stdDev ?? item?.standardDeviation),
    active: item?.active !== false,
    source: String(item?.source || "").trim(),
    correlationGroup: String(item?.correlationGroup || "").trim()
  };
}

function normalizeMonteCarloRisk(item, index = 0) {
  return {
    id: item?.id || "mc-risk-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7),
    name: String(item?.name || "Risiko " + (index + 1)).trim(),
    probability: Math.max(0, Math.min(1, Number(item?.probability) || 0)),
    minCostImpact: parseMonteCarloNumber(item?.minCostImpact),
    maxCostImpact: parseMonteCarloNumber(item?.maxCostImpact),
    minDelayDays: parseMonteCarloNumber(item?.minDelayDays),
    maxDelayDays: parseMonteCarloNumber(item?.maxDelayDays),
    active: item?.active !== false
  };
}
...
function buildMonteCarloConfig(state, options = {}) {
  ...
  const variablesSource = Array.isArray(monteCarloSource.variables) && monteCarloSource.variables.length
    ? monteCarloSource.variables
    : (Array.isArray(monteCarloSource.workPackages) ? monteCarloSource.workPackages.map((item, index) => ({
        id: item.id,
        key: normalizeMonteCarloKey(item.name || ("work_" + (index + 1)), index),
        name: item.name || ("Arbeitspaket " + (index + 1)),
        description: "Legacy-Arbeitspaket aus dem alten Monte-Carlo-Modell",
        unit: "€",
        distribution: "pert",
        minimum: Number(item.optimisticCost) || 0,
        likely: Number(item.mostLikelyCost) || 0,
        maximum: Number(item.pessimisticCost) || 0,
        mean: 0,
        stdDev: 0,
        active: true,
        source: "Legacy-Arbeitspaket"
      })) : []);
  const riskSource = Array.isArray(monteCarloSource.riskEvents) ? monteCarloSource.riskEvents : [];
  const variables = variablesSource.map((item, index) => normalizeMonteCarloVariable(item, index));
  const riskEvents = riskSource.map((item, index) => normalizeMonteCarloRisk(item, index));
  return {
    iterations: Math.max(1000, Number(monteCarloSource.iterations) || 10000),
    seed: monteCarloSource.seed || 42,
    targetValue: Number(monteCarloSource.targetValue ?? monteCarloSource.targetBudget ?? 0) || 0,
    targetLabel: String(monteCarloSource.targetLabel || "Zielwert").trim() || "Zielwert",
    targetUnit: String(monteCarloSource.targetUnit || "").trim(),
    modelName: String(monteCarloSource.modelName || "Monte-Carlo-Modell").trim(),
    objectiveName: String(monteCarloSource.objectiveName || "Zielgröße").trim(),
    formula: String(monteCarloSource.formula || "").trim(),
    formulaNote: String(monteCarloSource.formulaNote || "").trim(),
    scenarioMode: String(monteCarloSource.scenarioMode || "realistic").trim().toLowerCase(),
    scenarioNote: String(monteCarloSource.scenarioNote || "").trim(),
    correlationNote: String(monteCarloSource.correlationNote || "").trim(),
    variables,
    riskEvents,
    measureSettings
  };
}
function getMonteCarloScenarioMultiplier(mode) {
  switch (String(mode || "realistic").toLowerCase()) {
    case "optimistic":
      return 0.92;
    case "pessimistic":
      return 1.08;
...
function evaluateMonteCarloFormula(formula, sampledValues) {
  const keys = Object.keys(sampledValues || {});
  const values = keys.map((key) => sampledValues[key]);
  const source = String(formula || "").trim();
  if (!source) {
    return values.reduce((sum, value) => sum + (Number(value) || 0), 0);
  }
  try {
    const fn = new Function(...keys, "Math", `"use strict"; return (${source});`);
...
function computeMonteCarloCorrelation(xs, ys) {
  const xValues = (xs || []).filter((value) => Number.isFinite(value));
  const yValues = (ys || []).filter((value) => Number.isFinite(value));
  const length = Math.min(xValues.length, yValues.length);
  if (length < 2) return 0;
  const xMean = mean(xValues.slice(0, length));
  const yMean = mean(yValues.slice(0, length));
  let numerator = 0;
  let xVariance = 0;
  let yVariance = 0;
...
export function calculateMonteCarloResult(state, options = {}) {
  ...
  const outputRuns = [];
  const variableSamples = config.variables.map(() => []);
  const riskSamples = config.riskEvents.map(() => ({
    hits: 0,
    totalCostImpact: 0,
    totalDelayImpact: 0
  }));
  const scenarioMultiplier = getMonteCarloScenarioMultiplier(config.scenarioMode);
  const useDays = /tag/i.test(String(config.targetUnit || ""));
  const measurePercent = useDays
    ? Number(config.measureSettings?.daysPercent) || 0
    : Number(config.measureSettings?.budgetPercent) || 0;
  const measureFactor = config.applyMeasures ? Math.max(0, 1 - measurePercent / 100) : 1;
  const riskProbabilityFactor = config.applyMeasures ? Math.max(0, 1 - (Number(config.measureSettings?.riskReductionPercent) || 0) / 100) : 1;
  const riskImpactFactor = config.applyMeasures ? Math.max(0, 1 - (Number(config.measureSettings?.impactReductionPercent) || 0) / 100) : 1;

  for (let iteration = 0; iteration < iterations; iteration += 1) {
    const sampledValues = {};
    config.variables.forEach((variable, index) => {
      if (!variable.active) return;
      const sampledValue = sampleMonteCarloDistribution(variable, rng);
      sampledValues[variable.key] = sampledValue;
      variableSamples[index].push(sampledValue);
    });

    let outputValue = evaluateMonteCarloFormula(config.formula, sampledValues);

    config.riskEvents.forEach((risk, index) => {
      if (!risk.active) return;
      const probability = Math.max(0, Math.min(1, Number(risk.probability) || 0)) * riskProbabilityFactor;
      if (rng() <= probability) {
        const minImpact = useDays ? Number(risk.minDelayDays) || 0 : Number(risk.minCostImpact) || 0;
        const maxImpact = useDays ? Number(risk.maxDelayDays) || 0 : Number(risk.maxCostImpact) || 0;
        const sampledImpact = minImpact + rng() * Math.max(maxImpact - minImpact, 0);
        const adjustedImpact = sampledImpact * riskImpactFactor;
        outputValue += adjustedImpact;
        riskSamples[index].hits += 1;
        if (useDays) {
          riskSamples[index].totalDelayImpact += adjustedImpact;
        } else {
          riskSamples[index].totalCostImpact += adjustedImpact;
        }
      }
    });

    outputValue *= scenarioMultiplier;
    outputValue *= measureFactor;
    outputRuns.push(outputValue);
  }

  const meanValue = mean(outputRuns);
  const medianValue = percentile(outputRuns, 0.5);
  const p10Value = percentile(outputRuns, 0.1);
  const p50Value = percentile(outputRuns, 0.5);
  const p80Value = percentile(outputRuns, 0.8);
  const p90Value = percentile(outputRuns, 0.9);
  const minValue = outputRuns.length ? Math.min(...outputRuns) : 0;
  const maxValue = outputRuns.length ? Math.max(...outputRuns) : 0;
  const varianceValue = outputRuns.length
    ? outputRuns.reduce((sum, value) => sum + Math.pow(value - meanValue, 2), 0) / outputRuns.length
    : 0;
  const standardDeviationValue = Math.sqrt(varianceValue);
  const targetThreshold = Number(config.targetValue) || 0;
  const targetHits = targetThreshold > 0 ? outputRuns.filter((value) => value <= targetThreshold).length : 0;
  const targetSuccessRate = targetThreshold > 0 ? targetHits / Math.max(outputRuns.length, 1) : 0;

  const variableStats = config.variables.map((variable, index) => {
    const samples = variableSamples[index] || [];
    const sampleMean = samples.length ? mean(samples) : 0;
    const sampleStdDev = samples.length > 1
      ? Math.sqrt(samples.reduce((sum, value) => sum + Math.pow(value - sampleMean, 2), 0) / samples.length)
      : 0;
    return {
      ...variable,
      sampleMean,
      sampleStdDev,
      minObserved: samples.length ? Math.min(...samples) : 0,
      maxObserved: samples.length ? Math.max(...samples) : 0,
      activeShare: samples.length && meanValue ? sampleMean / meanValue : 0,
      correlation: computeMonteCarloCorrelation(samples, outputRuns),
      sampleCount: samples.length
    };
  });

  const riskResults = config.riskEvents.map((risk, index) => {
    const riskSample = riskSamples[index] || { hits: 0, totalCostImpact: 0, totalDelayImpact: 0 };
    const hitRate = iterations > 0 ? riskSample.hits / iterations : 0;
    const averageImpact = useDays
      ? (riskSample.totalDelayImpact || 0) / Math.max(riskSample.hits, 1)
      : (riskSample.totalCostImpact || 0) / Math.max(riskSample.hits, 1);
    return {
      ...risk,
      hitRate,
      avgImpact: averageImpact,
      exposureScore: averageImpact * hitRate
    };
  });
  ...
  return {
    ...
    scenarioNote: config.scenarioNote,
    correlationNote: config.correlationNote,
    outputRuns,
    variableSamples,
    variables: variableStats,
    riskEvents: riskResults,
    meanValue,
    medianValue,
    p10Value,
    p50Value,
    ...
    dayRuns: outputRuns,
    meanCost: meanValue,
    medianCost: medianValue,
    p10Cost: p10Value,
    p50Cost: p50Value,
    p80Cost: p80Value,
    p90Cost: p90Value,
    meanDays: meanValue,
    medianDays: medianValue,
    p10Days: p10Value,
    p50Days: p50Value,
    p80Days: p80Value,
    p90Days: p90Value,
    budgetSuccessRate: targetSuccessRate,
    scheduleSuccessRate: targetSuccessRate,
    combinedSuccessRate: targetSuccessRate
  };
}

export function calculateMonteCarloAnalytics(result) {
  const variableRows = (result.variables || [])
    .map((item, originalIndex) => ({
      ...item,
      originalIndex,
      absCorrelation: Math.abs(Number(item.correlation) || 0),
      influenceScore: Math.abs(Number(item.correlation) || 0) * (Number(item.sampleStdDev) || 0)
    }))
    .sort((a, b) => b.absCorrelation - a.absCorrelation || b.sampleMean - a.sampleMean);

  const riskRows = (result.riskEvents || [])
    .map((item, originalIndex) => ({
      ...item,
      originalIndex,
      exposureScore: Number(item.exposureScore) || 0
    }))
    .sort((a, b) => b.exposureScore - a.exposureScore);

  const outputDistributionSvg = createMonteCarloDistributionSvg(
    result.outputRuns,
    { p50: result.p50Value, p80: result.p80Value },
    {
      ariaLabel: `Verteilung des Zielwerts (${result.targetLabel || "Zielwert"})`,
      barColor: "#1e4f7b",
      formatter: (value) => formatMonteCarloValue(value, result.targetUnit)
    }
  );

  const variableInfluenceSvg = createPertBarChartSvg(
    variableRows,
    "absCorrelation",
    "#163f63",
    (value) => `${formatPercent(value)}`,
    { left: 172, right: 102, labelFontSize: 10.3, valueFontSize: 11 }
  );

  const riskImpactSvg = createPertBarChartSvg(
    riskRows,
    "avgImpact",
    "#8a6831",
    (value) => formatMonteCarloValue(value, result.targetUnit),
    { left: 172, right: 102, labelFontSize: 10.3, valueFontSize: 11 }
  );
  ...
  return {
    variableRows,
    riskRows,
    outputDistributionSvg,
    variableInfluenceSvg,
    riskImpactSvg
  };
}
...
export function buildManagementReportData(state) {
  ...
  const summaryCards = [
    ...
    {
      label: "Monte-Carlo Zieltreffer",
      value: formatPercent(monteCarlo.combinedSuccessRate),
      detail: `P80 Zielwert: ${formatMonteCarloValue(monteCarlo.p80Value, monteCarlo.targetUnit)}`,
      linkModule: "monteCarlo",
      linkTarget: "monte-p80-target",
      linkLabel: "P80 Zielwert im Monte-Carlo-Block anzeigen"
    },
    {
  ...
  if (monteCarlo.combinedSuccessRate < 0.4) {
    focusPoints.push(`Der Monte-Carlo-Zieltreffer liegt nur bei ${formatPercent(monteCarlo.combinedSuccessRate)} und ist damit derzeit kritisch.`);
  } else if (monteCarlo.combinedSuccessRate < 0.65) {
    focusPoints.push(`Der Monte-Carlo-Zieltreffer liegt bei ${formatPercent(monteCarlo.combinedSuccessRate)} und erfordert aktive Steuerung von Modell, Parametern und Reserven.`);
  } else {
    focusPoints.push(`Der Monte-Carlo-Zieltreffer liegt bei ${formatPercent(monteCarlo.combinedSuccessRate)} und ist aktuell vergleichsweise robust.`);
  }
  ...
  const nextSteps = [];
  if (monteCarlo.p80Value > monteCarlo.targetValue) {
    nextSteps.push(`Zielreserve prüfen und den Projektrahmen gegen das P80-Niveau von ${formatMonteCarloValue(monteCarlo.p80Value, monteCarlo.targetUnit)} spiegeln.`);
  }
...
export function renderRiskReportText(state) {
  ...
  if (sections.includeMonteCarlo) {
    lines.push(
      "",
      "Monte Carlo",
      `- Erwarteter Zielwert: ${formatMonteCarloValue(report.raw.monteCarlo.meanValue, report.raw.monteCarlo.targetUnit)}`,
      `- Median: ${formatMonteCarloValue(report.raw.monteCarlo.medianValue, report.raw.monteCarlo.targetUnit)}`,
      `- P80 Zielwert: ${formatMonteCarloValue(report.raw.monteCarlo.p80Value, report.raw.monteCarlo.targetUnit)}`,
      `- P90 Zielwert: ${formatMonteCarloValue(report.raw.monteCarlo.p90Value, report.raw.monteCarlo.targetUnit)}`,
      `- Zieltreffer: ${formatPercent(report.raw.monteCarlo.targetSuccessRate)}`
    );
  }
...
export function buildSelectedReportData(state) {
  ...
  if (sections.includeMonteCarlo) {
    payload.modules.monteCarlo = {
      targetLabel: report.raw.monteCarlo.targetLabel,
      targetUnit: report.raw.monteCarlo.targetUnit,
      targetValue: report.raw.monteCarlo.targetValue,
      meanValue: report.raw.monteCarlo.meanValue,
      medianValue: report.raw.monteCarlo.medianValue,
      p80Value: report.raw.monteCarlo.p80Value,
      p90Value: report.raw.monteCarlo.p90Value,
      targetSuccessRate: report.raw.monteCarlo.targetSuccessRate,
      ...
      variables: report.raw.monteCarlo.variables,
      riskEvents: report.raw.monteCarlo.riskEvents
    };
  }
...
export const modules = {
  ...
  monteCarlo: {
    ...
    subtitle: "Formelmodell, Verteilungen und Szenarien",
    render(state) {
      const result = calculateMonteCarloResult(state);
      const analytics = calculateMonteCarloAnalytics(result);
      const valueFormat = (value) => {
        if (String(result.targetUnit || "").includes("€")) return formatCurrency(value);
        if (/tag/i.test(String(result.targetUnit || ""))) return `${formatNumber(value)} Tage`;
        return `${formatNumber(value)} ${result.targetUnit || ""}`.trim();
      };
      const targetLabel = result.targetLabel || "Zielwert";
      const activeVariables = (result.variables || []).filter((item) => item.active !== false);
      const activeRisks = (result.riskEvents || []).filter((item) => item.active !== false);
      const scenarioLabel = {
        baseline: "Baseline",
        optimistic: "Optimistisch",
        realistic: "Realistisch",
        pessimistic: "Pessimistisch"
      }[result.scenarioMode] || "Individuell";
      const distributionHints = [
        "Dreieck: Minimum, wahrscheinlichster Wert, Maximum",
        "PERT: Minimum, wahrscheinlichster Wert, Maximum mit geglätteter Streuung",
        "Linear: Gleichverteilung im Intervall",
        "Normal: Mittelwert und Standardabweichung",
        "Lognormal: positive, rechtsschiefe Werte"
      ];
      const formulaKeys = activeVariables.map((item) => `<span class="monte-key-chip">${escapeHtml(item.key)}</span>`).join("");
        ...
        <div class="module-shell monte-module">
          <div class="module-title">
            <div>
              ...
              <p>Das Modul arbeitet jetzt als Formelmodell: Du definierst Zielgröße, Formel, Variablen und Verteilungen. Die Simulation wertet daraus eine Verteilung, Szenarien und die wichtigsten Einflussgrößen aus.</p>
            </div>
            <span class="badge">${scenarioLabel}</span>
          </div>

          <div class="kpi-grid">
            <article class="kpi-card" id="monte-p80-target"><div class="kpi-label">Erwarteter Zielwert</div><div class="kpi-value">${valueFormat(result.meanValue)}</div><div class="kpi-sub">Median: ${valueFormat(result.medianValue)} · P80: ${valueFormat(result.p80Value)}</div></article>
            <article class="kpi-card gold"><div class="kpi-label">P90-Zielwert</div><div class="kpi-value">${valueFormat(result.p90Value)}</div><div class="kpi-sub">Min: ${valueFormat(result.minValue)} · Max: ${valueFormat(result.maxValue)}</div></article>
            <article class="kpi-card green"><div class="kpi-label">Zieltreffer</div><div class="kpi-value">${formatPercent(result.targetSuccessRate)}</div><div class="kpi-sub">gegen ${escapeHtml(targetLabel)} ${result.targetValue ? `· ${valueFormat(result.targetValue)}` : ""}</div></article>
            <article class="kpi-card blue"><div class="kpi-label">Simulation</div><div class="kpi-value">${formatNumber(result.iterations)}</div><div class="kpi-sub">Seed ${formatNumber(result.seed)} · ${activeVariables.length} Variablen</div></article>
          </div>
          ...
          <div class="card-grid monte-upper-grid">
            <section class="info-card">
              <div class="monte-section-head">
                <div>
                  <h3>Modellkopf</h3>
                  <p>Hier definierst du die Zielgröße und die Formel, die in jeder Iteration ausgewertet wird.</p>
                </div>
              </div>
              <div class="monte-control-grid monte-model-grid">
                <div class="monte-control-field">
                  <label for="mc_model_name">Modellname</label>
                  <input class="compact-input" id="mc_model_name" data-mc-model-field="modelName" type="text" value="${escapeHtml(result.modelName)}">
                </div>
                <div class="monte-control-field">
                  <label for="mc_objective_name">Zielgröße</label>
                  <input class="compact-input" id="mc_objective_name" data-mc-model-field="objectiveName" type="text" value="${escapeHtml(result.objectiveName)}">
                </div>
                <div class="monte-control-field">
                  <label for="mc_target_label">Zielwert-Label</label>
                  <input class="compact-input" id="mc_target_label" data-mc-model-field="targetLabel" type="text" value="${escapeHtml(result.targetLabel)}">
                </div>
                <div class="monte-control-field">
                  <label for="mc_target_unit">Einheit</label>
                  <input class="compact-input" id="mc_target_unit" data-mc-model-field="targetUnit" type="text" value="${escapeHtml(result.targetUnit)}">
                </div>
                <div class="monte-control-field" style="grid-column:1 / -1;">
                  <label for="mc_formula">Formel</label>
                  <input class="compact-input" id="mc_formula" data-mc-model-field="formula" type="text" value="${escapeHtml(result.formula)}">
                </div>
                <div class="monte-control-field" style="grid-column:1 / -1;">
                  <label for="mc_formula_note">Formelhinweis</label>
                  <input class="compact-input" id="mc_formula_note" data-mc-model-field="formulaNote" type="text" value="${escapeHtml(result.formulaNote)}">
                </div>
              </div>
              <div class="monte-control-metrics">
                <div class="monte-control-stat">
                  <span>Aktive Variablen</span>
                  <strong>${formatNumber(activeVariables.length)}</strong>
                  <small>Schlüssel: ${formulaKeys || "noch keine"}</small>
                </div>
                <div class="monte-control-stat">
                  <span>Simulationsmodus</span>
                  <strong>${scenarioLabel}</strong>
                  <small>Aktuell: ${escapeHtml(result.scenarioMode)}</small>
                </div>
                <div class="monte-control-stat">
                  <span>Konfidenz-Punkt</span>
                  <strong>P80</strong>
                  <small>${valueFormat(result.p80Value)}</small>
                </div>
                <div class="monte-control-stat">
                  <span>Standardabweichung</span>
                  <strong>${valueFormat(result.standardDeviationValue)}</strong>
                  <small>Streuung der Zielwertverteilung</small>
                </div>
              </div>
            </section>
            ...
            <section class="info-card">
              <div class="monte-section-head">
                <div>
                  <h3>Simulation und Szenarien</h3>
                  <p>Steuere die Rechenläufe und sichere belastbare Modellstände als Baseline oder Szenario.</p>
                </div>
                <span class="badge">Szenarien</span>
              </div>
              <div class="monte-control-grid">
                <div class="monte-control-field">
                  <label for="mc_iterations">Iterationen</label>
                  <input class="compact-input integer-input" id="mc_iterations" data-mc-field="iterations" type="text" inputmode="numeric" value="${formatIntegerInput(result.iterations)}">
                </div>
                <div class="monte-control-field">
                  <label for="mc_seed">Seed</label>
                  <input class="compact-input integer-input" id="mc_seed" data-mc-field="seed" type="text" inputmode="numeric" value="${formatIntegerInput(result.seed)}">
                </div>
                <div class="monte-control-field">
                  <label for="mc_target_value">Grenzwert</label>
                  <input class="compact-input currency-input" id="mc_target_value" data-mc-model-field="targetValue" type="text" inputmode="numeric" value="${formatCurrencyInput(result.targetValue)}">
                </div>
                <div class="monte-control-field">
                  <label for="mc_scenario_mode">Szenario</label>
                  <select class="compact-input" id="mc_scenario_mode" data-mc-model-field="scenarioMode">
                    ${["baseline", "optimistic", "realistic", "pessimistic"].map((option) => `<option value="${option}" ${result.scenarioMode === option ? "selected" : ""}>${scenarioLabel === option ? scenarioLabel : option.charAt(0).toUpperCase() + option.slice(1)}</option>`).join("")}
                  </select>
                </div>
              </div>
              <div class="control-actions" style="margin-top:14px;flex-wrap:wrap;gap:10px;">
                ...
                <button class="action-btn" type="button" data-action="restore-mc-baseline" ${state.monteCarlo.baselineScenario ? "" : "disabled"}>Baseline wiederherstellen</button>
              </div>
              <div class="monte-baseline-panel ${state.monteCarlo.baselineScenario ? "is-available" : "is-empty"}" style="margin-top:12px;">
                ...
                <div class="monte-baseline-metrics">
                  ...
                  <div><span>Variablen</span><strong>${formatNumber(activeVariables.length)}</strong></div>
                  <div><span>Risiken</span><strong>${formatNumber(activeRisks.length)}</strong></div>
                </div>
              </div>
              ${(state.monteCarlo.storedScenarios || []).length ? `
                <div style="margin-top:14px;">
                  <div class="monte-section-head" style="margin-bottom:10px;">
                    <div>
                      <h3>Gesicherte Szenarien</h3>
                      <p>Die Szenarien speichern Formel, Variablen und Simulationsergebnisse des jeweiligen Modellstands.</p>
                    </div>
                  </div>
                    ...
                    ${state.monteCarlo.storedScenarios.map((scenario, index) => `
                      <div class="mini-card monte-scenario-card">
                        <div class="monte-scenario-card-head">
                          ...
                          <p>${escapeHtml(scenario.savedAt)}</p>
                        </div>
                        <div class="monte-scenario-metrics">
                          <div><span>Zielwert</span><strong>${scenario.summary ? valueFormat(scenario.summary.meanValue) : "—"}</strong></div>
                          <div><span>P80</span><strong>${scenario.summary ? valueFormat(scenario.summary.p80Value) : "—"}</strong></div>
                          <div><span>Treffer</span><strong>${scenario.summary ? formatPercent(scenario.summary.targetSuccessRate || 0) : "—"}</strong></div>
          ...
          <section class="info-card">
            <div class="monte-section-head">
              <div>
                <h3>Variablen</h3>
                <p>Jede Variable bekommt eine Verteilung und die passenden Parameter. Daraus entsteht die Zielwertverteilung.</p>
              </div>
              <button class="action-btn add-with-icon" type="button" data-action="add-mc-variable"><span class="add-icon" aria-hidden="true">+</span><span>Variable hinzufügen</span></button>
            </div>
            <div class="form-note" style="margin-top:0;">
              Verteilungen: ${distributionHints.join(" · ")}
            </div>
            <div class="monte-edit-stack" style="margin-top:12px;">
              ${result.variables.map((variable) => `
                <details class="monte-edit-card monte-collapsible-card variable-card ${variable.active ? "" : "is-muted"}" data-monte-variable-id="${variable.id}">
                    ...
                    <div class="monte-card-summary-main">
                      <div class="monte-card-summary-topline">
                        ...
                        <div class="monte-card-summary-title">
                          ...
                          <span>${escapeHtml(variable.key)} · ${escapeHtml(variable.distribution)} · ${escapeHtml(variable.unit || "ohne Einheit")}</span>
                        </div>
                      </div>
                      <div class="monte-card-summary-metrics">
                        <span class="monte-card-metric"><small>Mittelwert</small><strong>${valueFormat(variable.sampleMean)}</strong></span>
                        <span class="monte-card-metric"><small>Streuung</small><strong>${valueFormat(variable.sampleStdDev)}</strong></span>
                        <span class="monte-card-metric"><small>Einfluss</small><strong>${formatPercent(Math.abs(variable.correlation) || 0)}</strong></span>
                        <span class="monte-card-metric"><small>Aktiv</small><strong>${variable.active ? "Ja" : "Nein"}</strong></span>
                      </div>
                    </div>
                    <div class="monte-card-summary-actions">
                      <label style="display:flex;align-items:center;gap:6px;font-size:0.8rem;color:var(--text-secondary);margin-right:8px;">
                        <input type="checkbox" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="active" ${variable.active ? "checked" : ""}>
                        Aktiv
                      </label>
                      <button class="danger-icon-btn" type="button" aria-label="Variable entfernen" data-action="remove-mc-variable" data-index="${variable.originalIndex}">×</button>
                    </div>
                  </summary>
                  <div class="monte-card-content">
                    <div class="monte-edit-card-head">
                      <div class="monte-edit-title">
                        <label for="mc_var_name_${variable.originalIndex}">Variable</label>
                        <input class="compact-input name" id="mc_var_name_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="name" type="text" value="${escapeHtml(variable.name)}">
                      </div>
                    </div>
                    <div class="monte-edit-group-stack">
                      <section class="monte-edit-group">
                        <div class="monte-edit-group-head">
                          <h4>Beschreibung und Verteilung</h4>
                          <p>Lege fest, wie die Variable im Modell schwankt.</p>
                        </div>
                        <div class="monte-edit-grid monte-variable-grid">
                          <div class="form-field">
                            <label for="mc_var_key_${variable.originalIndex}">Schlüssel</label>
                            <input class="compact-input" id="mc_var_key_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="key" type="text" value="${escapeHtml(variable.key)}">
                          </div>
                          <div class="form-field">
                            <label for="mc_var_unit_${variable.originalIndex}">Einheit</label>
                            <input class="compact-input" id="mc_var_unit_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="unit" type="text" value="${escapeHtml(variable.unit || "")}">
                          </div>
                          <div class="form-field">
                            <label for="mc_var_dist_${variable.originalIndex}">Verteilung</label>
                            <select class="compact-input" id="mc_var_dist_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="distribution">
                              ${["uniform", "linear", "triangular", "pert", "normal", "lognormal"].map((option) => `<option value="${option}" ${String(variable.distribution || "uniform").toLowerCase() === option ? "selected" : ""}>${option === "uniform" ? "Gleichverteilung" : option === "linear" ? "Linear" : option === "triangular" ? "Dreieck" : option === "pert" ? "PERT" : option === "normal" ? "Normal" : "Lognormal"}</option>`).join("")}
                            </select>
                          </div>
                          <div class="form-field">
                            <label for="mc_var_source_${variable.originalIndex}">Quelle</label>
                            <input class="compact-input" id="mc_var_source_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="source" type="text" value="${escapeHtml(variable.source || "")}">
                          </div>
                          <div class="form-field" style="grid-column:1 / -1;">
                            <label for="mc_var_description_${variable.originalIndex}">Beschreibung</label>
                            <input class="compact-input" id="mc_var_description_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="description" type="text" value="${escapeHtml(variable.description || "")}">
                          </div>
                        </div>
                      </section>
                      <section class="monte-edit-group">
                        <div class="monte-edit-group-head">
                          <h4>Verteilungsparameter</h4>
                          <p>Die Felder sind für alle Standardverteilungen geeignet.</p>
                        </div>
                        <div class="monte-edit-grid monte-variable-grid">
                          <div class="form-field">
                            <label for="mc_var_min_${variable.originalIndex}">Minimum</label>
                            <input class="compact-input currency-input" id="mc_var_min_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="minimum" type="text" inputmode="numeric" value="${formatCurrencyInput(variable.minimum)}">
                          </div>
                          <div class="form-field">
                            <label for="mc_var_likely_${variable.originalIndex}">Wahrscheinlich</label>
                            <input class="compact-input currency-input" id="mc_var_likely_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="likely" type="text" inputmode="numeric" value="${formatCurrencyInput(variable.likely)}">
                          </div>
                          <div class="form-field">
                            <label for="mc_var_max_${variable.originalIndex}">Maximum</label>
                            <input class="compact-input currency-input" id="mc_var_max_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="maximum" type="text" inputmode="numeric" value="${formatCurrencyInput(variable.maximum)}">
                          </div>
                          <div class="form-field">
                            <label for="mc_var_mean_${variable.originalIndex}">Mittelwert</label>
                            <input class="compact-input currency-input" id="mc_var_mean_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="mean" type="text" inputmode="numeric" value="${formatCurrencyInput(variable.mean)}">
                          </div>
                          <div class="form-field">
                            <label for="mc_var_std_${variable.originalIndex}">Std.-Abw.</label>
                            <input class="compact-input currency-input" id="mc_var_std_${variable.originalIndex}" data-mc-variable-index="${variable.originalIndex}" data-mc-variable-field="stdDev" type="text" inputmode="numeric" value="${formatCurrencyInput(variable.stdDev)}">
                          </div>
                        </div>
                      </section>
                    </div>
                    <div class="monte-edit-metrics monte-variable-result-row">
                      <div><span>Stichproben-Mittel</span><strong>${valueFormat(variable.sampleMean)}</strong></div>
                      <div><span>Stichproben-Streuung</span><strong>${valueFormat(variable.sampleStdDev)}</strong></div>
                      <div><span>Korrelation zum Ergebnis</span><strong>${formatPercent(Math.abs(variable.correlation) || 0)}</strong></div>
                      <div><span>Aktiv</span><strong>${variable.active ? "Ja" : "Nein"}</strong></div>
                    </div>
                  </div>
                </details>
              `).join("")}
          ...
          <section class="info-card">
            <div class="monte-section-head">
              <div>
                <h3>Risikomodifikatoren</h3>
                <p>Risikofaktoren ergänzen die Formel als probabilistische Aufschläge. Du kannst Kosten- oder Terminwirkungen hinterlegen.</p>
              </div>
            ...
            <div class="monte-edit-stack" style="margin-top:12px;">
              ${result.riskEvents.map((risk) => `
                <details class="monte-edit-card monte-collapsible-card risk-card ${risk.active ? "" : "is-muted"}" data-monte-risk-id="${risk.id}">
                  <summary class="monte-card-summary">
                    <div class="monte-card-summary-main">
                      <div class="monte-card-summary-topline">
                        <span class="monte-card-toggle" aria-hidden="true"></span>
                        <div class="monte-card-summary-title">
                          ...
                          <span>${formatPercent(risk.probability)} · Ø ${valueFormat(risk.avgImpact || 0)}</span>
                        </div>
                      </div>
                      <div class="monte-card-summary-metrics">
                        <span class="monte-card-metric"><small>Eintritt</small><strong>${formatPercent(risk.probability)}</strong></span>
                        <span class="monte-card-metric"><small>Ø Wirkung</small><strong>${valueFormat(risk.avgImpact || 0)}</strong></span>
                        <span class="monte-card-metric"><small>Treffer</small><strong>${formatPercent(risk.hitRate || 0)}</strong></span>
                        <span class="monte-card-metric"><small>Aktiv</small><strong>${risk.active ? "Ja" : "Nein"}</strong></span>
                      </div>
                    </div>
                    <div class="monte-card-summary-actions">
                      <label style="display:flex;align-items:center;gap:6px;font-size:0.8rem;color:var(--text-secondary);margin-right:8px;">
                        <input type="checkbox" data-mc-risk-index="${risk.originalIndex}" data-mc-risk-field="active" ${risk.active ? "checked" : ""}>
                        Aktiv
                      </label>
                  ...
                  <div class="monte-card-content">
                    <div class="monte-edit-card-head">
                      <div class="monte-edit-title">
                        ...
                        <input class="compact-input name" id="mc_risk_name_${risk.originalIndex}" data-mc-risk-index="${risk.originalIndex}" data-mc-risk-field="name" type="text" value="${escapeHtml(risk.name)}">
                      </div>
                    </div>
                    <div class="monte-edit-group-stack">
                      <section class="monte-edit-group">
                        <div class="monte-edit-group-head">
                          <h4>Eintritt und Wirkung</h4>
                          <p>Risiken wirken probabilistisch auf die Zielgröße.</p>
                        </div>
                        <div class="monte-edit-grid monte-risk-grid">
                          ...
                          <div class="form-field">
                            <label for="mc_risk_min_cost_${risk.originalIndex}">Wirkung minimum</label>
                          ...
                          <div class="form-field">
                            <label for="mc_risk_max_cost_${risk.originalIndex}">Wirkung maximum</label>
                          ...
                          <div class="form-field">
                            <label for="mc_risk_min_days_${risk.originalIndex}">Termin minimum</label>
                          ...
                          <div class="form-field">
                            <label for="mc_risk_max_days_${risk.originalIndex}">Termin maximum</label>
          ...
          <div class="card-grid monte-distribution-grid">
            <section class="chart-box monte-distribution-card">
              <h3>Verteilung des Zielwerts</h3>
              <p class="chart-intro">Histogramm aller Simulationsläufe für die Zielgröße. P50 und P80 markieren die zentrale Steuerungsspanne.</p>
              ${analytics.outputDistributionSvg}
            </section>
            <section class="chart-box monte-distribution-card">
              <h3>Einfluss der Variablen</h3>
              <p class="chart-intro">Die Korrelation zeigt, welche Variablen die Zielwertverteilung am stärksten treiben.</p>
              ${analytics.variableInfluenceSvg}
            </section>
          </div>
          ...
          <section class="info-card">
            <div class="monte-section-head">
              <div>
                <h3>Risikowirkung</h3>
                <p>Die Risikofaktoren sind mit ihrer Eintrittswahrscheinlichkeit und mittleren Wirkung verdichtet dargestellt.</p>
              </div>
              <span class="badge">${formatNumber(activeRisks.length)} Risiken</span>
            </div>
            <div class="monte-feature-layout">
              <div class="chart-box compact-chart-box monte-feature-chart">
                ${analytics.riskImpactSvg}
              </div>
                ...
                <table class="data-table">
                  ...
                  <tbody>
                    ${analytics.riskRows.map((risk) => `
                      <tr>
                        <td><strong>${escapeHtml(risk.name)}</strong></td>
                        <td class="numeric-output">${formatPercent(risk.probability)}</td>
                        <td class="numeric-output">${valueFormat(risk.avgImpact || 0)}</td>
                        <td class="numeric-output">${formatPercent(risk.hitRate || 0)}</td>
                      </tr>
                    `).join("")}
                  </tbody>
                </table>
          ...
          <section class="info-card">
            <div class="monte-section-head">
              <div>
                ...
                <p>Die wichtigsten Kennzahlen der Simulation für Steuerung und Bericht.</p>
              </div>
            </div>
            <div class="monte-control-metrics" style="margin-top:0;">
              <div class="monte-control-stat">
                <span>Erwartung</span>
                <strong>${valueFormat(result.meanValue)}</strong>
                <small>arithmetisches Mittel</small>
              </div>
              <div class="monte-control-stat">
                <span>Median</span>
                <strong>${valueFormat(result.medianValue)}</strong>
                <small>P50 der Verteilung</small>
              </div>
              <div class="monte-control-stat">
                <span>P80</span>
                <strong>${valueFormat(result.p80Value)}</strong>
                <small>konservative Steuerung</small>
              </div>
              <div class="monte-control-stat">
                <span>Trefferquote</span>
                <strong>${formatPercent(result.targetSuccessRate)}</strong>
                <small>unter dem definierten Grenzwert</small>
              </div>
            </div>
            <div class="analysis-grid monte-baseline-compare-grid" style="margin-top:14px;">
              <div class="mini-card baseline-highlight">
                <h4>Formel</h4>
                <div class="form-note" style="margin-bottom:0;">${escapeHtml(result.formula || "noch keine Formel definiert")}</div>
              </div>
              <div class="mini-card baseline-highlight">
                <h4>Hinweis</h4>
                <div class="form-note" style="margin-bottom:0;">${escapeHtml(result.formulaNote || "Ergänze hier die fachliche Erklärung zum Modell.")}</div>
              </div>
            </div>
          </section>
        </div>
      ...
      return `
        <div class="module-shell risk-register-shell">
          ...
          <details class="info-card risk-fold-card ai-workshop-card" open style="margin-top:18px;">
            ...
            <p class="form-note" style="margin-top:8px;">Mit dem Übernehmen-Button speicherst du den geprüften Vorschlag im Risikoregister und prüfst fachlich im Risiko-Dashboard, ob alle Inhalte korrekt wiedergegeben sind. Passe sie bei Bedarf an und optimiere den Vorschlag, da KI-generierte Inhalte Fehlinterpretationen enthalten können.</p>
            </div>
          </details>
