import { initialState } from "./state.r268.js";
...
function buildRiskCategoryOptions(currentValue) {
  ...
  const options = [...riskCategoryOptions];
  if (current && !options.some((option) => String(option).trim().toLowerCase() === currentLower)) {
    options.splice(options.length - 1, 0, current);
  }
  return options;
}
