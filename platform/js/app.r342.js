...
let draggedEvaPanelKey = null;
let draggedEvaPanelPointerId = null;
let draggedEvaPanelElement = null;
let draggedEvaPanelStartX = 0;
let draggedEvaPanelStartY = 0;
...
function normalizeEvaActivityUndoStack(stack) {
  if (!Array.isArray(stack)) return [];
  return stack
...
function normalizeEvaActivityRedoStack(stack) {
  if (!Array.isArray(stack)) return [];
  return stack
    .filter((entry) => Array.isArray(entry))
    .map((entry) => JSON.parse(JSON.stringify(entry)))
...
function pushEvaActivityUndoSnapshot(state) {
  const currentActivities = JSON.parse(JSON.stringify(state.earnedValue?.activities || []));
  const currentStack = normalizeEvaActivityUndoStack(state.ui?.evaActivityUndoStack);
  const nextStack = [...currentStack, currentActivities].slice(-20);
  store.setState((current) => ({
    ...current,
    ui: {
      ...current.ui,
      evaActivityUndoStack: nextStack,
      evaActivityRedoStack: []
    }
  }));
}

function applyEvaActivityUndo() {
  const state = store.getState();
  const stack = normalizeEvaActivityUndoStack(state.ui?.evaActivityUndoStack);
  if (!stack.length) return false;
  const previousActivities = stack[stack.length - 1];
  const nextStack = stack.slice(0, -1);
  const currentActivities = JSON.parse(JSON.stringify(state.earnedValue?.activities || []));
  const redoStack = normalizeEvaActivityRedoStack(state.ui?.evaActivityRedoStack);
  const nextRedoStack = [...redoStack, currentActivities].slice(-20);
  store.setState((current) => ({
    ...current,
    earnedValue: {
      ...current.earnedValue,
      activities: JSON.parse(JSON.stringify(previousActivities))
    },
    ui: {
      ...current.ui,
      evaActivityUndoStack: nextStack,
      evaActivityRedoStack: nextRedoStack,
      evaActivityOpenIds: normalizeEvaActivityOpenIds(
        current.ui?.evaActivityOpenIds,
        previousActivities
      )
    }
  }));
...
function applyEvaActivityRedo() {
  const state = store.getState();
  const stack = normalizeEvaActivityRedoStack(state.ui?.evaActivityRedoStack);
  if (!stack.length) return false;
  const nextActivities = stack[stack.length - 1];
  const nextRedoStack = stack.slice(0, -1);
  const currentActivities = JSON.parse(JSON.stringify(state.earnedValue?.activities || []));
  const undoStack = normalizeEvaActivityUndoStack(state.ui?.evaActivityUndoStack);
  const nextUndoStack = [...undoStack, currentActivities].slice(-20);
  store.setState((current) => ({
    ...current,
    earnedValue: {
      ...current.earnedValue,
      activities: JSON.parse(JSON.stringify(nextActivities))
    },
    ui: {
      ...current.ui,
      evaActivityUndoStack: nextUndoStack,
      evaActivityRedoStack: nextRedoStack,
      evaActivityOpenIds: normalizeEvaActivityOpenIds(
        current.ui?.evaActivityOpenIds,
        nextActivities
...
function createNeutralProjectState() {
  ...
  next.monteCarlo = {
    ...next.monteCarlo,
    modelName: "Gesamtkostenmodell",
    objectiveName: "Projektkosten",
    targetLabel: "Projektbudget",
    targetUnit: "€",
    targetValue: 0,
    formula: "basis + material + personal + risiko",
    formulaNote: "",
  ...
  next.ui = {
    ...
    evaActivityOpenIds: [],
    evaActivitySortBy: "newest",
    evaActivityUndoStack: [],
    evaActivityRedoStack: [],
    reportOptions: {
      includePert: true,
    ...
    evaPanelOrder: ["overview", "activities", "trend", "management", "actions", "charts", "secondary", "table", "knowledge"],
    evaPanelOpenStates: {
      overview: true,
      trend: true,
      management: true,
...
function normalizeEvaUiState(ui = {}, activities = []) {
  const evaPanelOpenStates = normalizeEvaPanelOpenStates(ui.evaPanelOpenStates);
  if (ui.evaActivitiesPanelOpen === true) {
    evaPanelOpenStates.activities = true;
  }
  return {
    ...ui,
    ...
    evaPanelOrder: normalizeEvaPanelOrder(ui.evaPanelOrder),
    evaPanelOpenStates,
    evaActivitiesPanelOpen: ui.evaActivitiesPanelOpen === true || ui.evaPanelOpenStates?.activities === true,
    evaActivitySortBy: ui.evaActivitySortBy === "name" ? "name" : "newest",
    ...
    riskRegisterView: {
      ...
      topSortBy: ["priority", "value"].includes(ui.riskRegisterView?.topSortBy)
        ? ui.riskRegisterView.topSortBy
        : "priority",
      ...
      editSortBy: ["newest", "id"].includes(ui.riskRegisterView?.editSortBy)
        ...
        : "newest",
      panelOrder: normalizeRiskRegisterPanelOrder(ui.riskRegisterView?.panelOrder),
      panelOpenStates: normalizeRiskRegisterPanelOpenStates(ui.riskRegisterView?.panelOpenStates),
    ...
    riskRegisterRedoStack: normalizeRiskRegisterRedoStack(ui.riskRegisterRedoStack),
    evaActivityUndoStack: normalizeEvaActivityUndoStack(ui.evaActivityUndoStack),
    evaActivityRedoStack: normalizeEvaActivityRedoStack(ui.evaActivityRedoStack),
...
function normalizeRiskRegisterPanelOrder(panelOrder) {
  const defaultOrder = ["overview", "ai", "edit", "table", "chart", "matrix"];
  if (!Array.isArray(panelOrder)) return defaultOrder;
  const filtered = panelOrder.map((value) => String(value || "").trim()).filter((value) => defaultOrder.includes(value));
  const trailing = defaultOrder.filter((value) => !filtered.includes(value));
...
function normalizeEvaPanelOrder(panelOrder) {
  const defaultOrder = ["overview", "trend", "management", "actions", "charts", "secondary", "table", "activities", "knowledge"];
  if (!Array.isArray(panelOrder)) return defaultOrder;
  const filtered = panelOrder.map((value) => String(value || "").trim()).filter((value) => defaultOrder.includes(value));
  const trailing = defaultOrder.filter((value) => !filtered.includes(value));
  return [...filtered, ...trailing];
...
function normalizeEvaPanelOpenStates(panelOpenStates) {
  const defaultOpenStates = {
    overview: true,
    trend: true,
    management: true,
...
function getEvaPanelsByPosition() {
  return [...document.querySelectorAll?.("[data-eva-panel-key]") || []]
    .filter((panel) => panel instanceof HTMLElement)
    .map((panel) => ({ panel, rect: panel.getBoundingClientRect() }))
    .filter(({ rect }) => rect && rect.width > 0 && rect.height > 0)
    .sort((a, b) => a.rect.top - b.rect.top);
...
function renderNav(state) {
  ...
  if (!target) return;
  const allowedKeys = Array.isArray(state.ui?.visibleModules) && state.ui.visibleModules.length
    ? new Set(state.ui.visibleModules)
    : null;
  const navModules = Object.values(modules).filter((module) => !allowedKeys || allowedKeys.has(module.key));
...
function renderView(state) {
  ...
  const allowedKeys = Array.isArray(state.ui?.visibleModules) && state.ui.visibleModules.length
    ? new Set(state.ui.visibleModules)
    : null;
  const activeKey = allowedKeys && !allowedKeys.has(state.ui.activeModule)
    ? (state.ui.visibleModules[0] || state.ui.activeModule)
...
function createMonteCarloVariable() {
  return {
    id: `mc-var-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    key: `var_${Date.now().toString(36).slice(-4)}`,
    name: "Neue Variable",
    description: "",
    unit: "€",
...
function createMonteCarloScenario(state, type = "scenario") {
  ...
  return {
    ...
    targetValue: state.monteCarlo.targetValue,
    targetLabel: state.monteCarlo.targetLabel,
    targetUnit: state.monteCarlo.targetUnit,
    iterations: state.monteCarlo.iterations,
    seed: state.monteCarlo.seed,
    formula: state.monteCarlo.formula,
    modelName: state.monteCarlo.modelName,
    objectiveName: state.monteCarlo.objectiveName,
    scenarioMode: state.monteCarlo.scenarioMode,
    summary: {
      meanValue: summary.meanValue,
      medianValue: summary.medianValue,
      p50Value: summary.p50Value,
      p80Value: summary.p80Value,
      p90Value: summary.p90Value,
      targetSuccessRate: summary.targetSuccessRate
    },
    measureSettings: JSON.parse(JSON.stringify(state.monteCarlo.measureSettings || {})),
    variables: JSON.parse(JSON.stringify(state.monteCarlo.variables || [])),
...
function bindEvents() {
  ...
  document.addEventListener("pointerup", (event) => {
    ...
    if (!panel) {
      ...
      if (draggedRiskPanelElement) draggedRiskPanelElement.style.pointerEvents = "";
    ...
    store.setState((state) => {
      ...
      const nextOrder = moveItemRelative(currentOrder, draggedRiskPanelKey, targetKey, placeAfter);
      return {
        ...state,
        ui: {
          ...state.ui,
  ...
  document.addEventListener("pointerdown", (event) => {
    const handle = event.target?.closest?.("[data-eva-panel-drag-handle]");
    if (!handle || event.button !== 0) return;
    const panel = handle.closest("[data-eva-panel-key]");
    const panelKey = panel?.getAttribute("data-eva-panel-key");
    if (!panelKey) return;
    draggedEvaPanelKey = panelKey;
    draggedEvaPanelPointerId = event.pointerId ?? null;
    draggedEvaPanelElement = panel;
    draggedEvaPanelStartX = event.clientX ?? 0;
    draggedEvaPanelStartY = event.clientY ?? 0;
    panel.style.pointerEvents = "none";
    panel?.classList?.add?.("is-dragging");
    document.body?.classList?.add?.("eva-panel-dragging");
    if (typeof handle.setPointerCapture === "function" && draggedEvaPanelPointerId !== null) {
      try {
        handle.setPointerCapture(draggedEvaPanelPointerId);
      } catch (_error) {
        // Ignore pointer capture errors.
      }
    }
  ...
  document.addEventListener("pointermove", (event) => {
    if (!draggedEvaPanelElement) return;
    if (draggedEvaPanelPointerId !== null && event.pointerId !== draggedEvaPanelPointerId) return;
    const dx = (event.clientX ?? 0) - draggedEvaPanelStartX;
    const dy = (event.clientY ?? 0) - draggedEvaPanelStartY;
    draggedEvaPanelElement.style.transform = `translate3d(${dx}px, ${dy}px, 0)`;
  }, true);

  document.addEventListener("pointerup", (event) => {
    if (!draggedEvaPanelKey) return;
    const targetElement = document.elementFromPoint(event.clientX, event.clientY);
    let panel = targetElement?.closest?.("[data-eva-panel-key]");
    const panelsByPosition = getEvaPanelsByPosition();
    if (!panel && panelsByPosition.length) {
      const y = event.clientY ?? 0;
      const first = panelsByPosition[0];
      const last = panelsByPosition[panelsByPosition.length - 1];
    ...
    if (!panel) {
      draggedEvaPanelElement?.classList?.remove?.("is-dragging");
      if (draggedEvaPanelElement) draggedEvaPanelElement.style.pointerEvents = "";
      document.body?.classList?.remove?.("eva-panel-dragging");
      draggedEvaPanelKey = null;
      draggedEvaPanelPointerId = null;
      draggedEvaPanelElement = null;
      return;
    }
    const targetKey = panel.getAttribute("data-eva-panel-key");
    if (!targetKey || targetKey === draggedEvaPanelKey) {
      draggedEvaPanelElement?.classList?.remove?.("is-dragging");
      if (draggedEvaPanelElement) draggedEvaPanelElement.style.pointerEvents = "";
      document.body?.classList?.remove?.("eva-panel-dragging");
      draggedEvaPanelKey = null;
    ...
    const placeAfter = event.clientY > rect.top + (rect.height / 2);
    store.setState((state) => {
      const currentOrder = normalizeEvaPanelOrder(state.ui?.evaPanelOrder);
      const nextOrder = moveItemRelative(currentOrder, draggedEvaPanelKey, targetKey, placeAfter);
      return {
        ...state,
        ui: {
          ...state.ui,
          evaPanelOrder: nextOrder
        }
      };
    });
    draggedEvaPanelElement?.classList?.remove?.("is-dragging");
    if (draggedEvaPanelElement) draggedEvaPanelElement.style.pointerEvents = "";
    if (draggedEvaPanelElement) {
      draggedEvaPanelElement.style.transform = "";
    }
    document.body?.classList?.remove?.("eva-panel-dragging");
    draggedEvaPanelKey = null;
    draggedEvaPanelPointerId = null;
  ...
  document.addEventListener("toggle", (event) => {
    ...
    if (details.matches("[data-eva-panel-key]")) {
      const panelKey = details.getAttribute("data-eva-panel-key");
      if (!panelKey) return;
      const currentOpenStates = normalizeEvaPanelOpenStates(store.getState().ui?.evaPanelOpenStates);
      if (currentOpenStates[panelKey] === details.open) return;
      store.setState((state) => ({
        ...state,
        ui: {
          ...state.ui,
          evaPanelOpenStates: {
            ...normalizeEvaPanelOpenStates(state.ui?.evaPanelOpenStates),
            [panelKey]: details.open
          },
          evaActivitiesPanelOpen: panelKey === "activities" ? details.open : state.ui?.evaActivitiesPanelOpen === true
        }
      }));
      return;
    }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (actionTarget?.dataset.action === "reset-risk-register-panel-order") {
      store.setState((state) => ({
        ...
        ui: {
          ...
          riskRegisterView: {
            ...state.ui.riskRegisterView,
            panelOrder: ["overview", "ai", "edit", "table", "chart", "matrix"],
            panelOpenStates: {
              overview: false,
              ai: false,
              edit: false,
    ...
    if (actionTarget?.dataset.action === "close-eva-panels") {
      store.setState((state) => ({
        ...state,
        ui: {
          ...state.ui,
    ...
    if (actionTarget?.dataset.action === "reset-eva-panel-order") {
      store.setState((state) => ({
        ...state,
        ui: {
          ...state.ui,
    ...
    if (actionTarget?.dataset.action === "add-eva-activity") {
      pushEvaActivityUndoSnapshot(store.getState());
      store.setState((state) => ({
        ...state,
        earnedValue: {
          ...state.earnedValue,
    ...
    if (actionTarget?.dataset.action === "undo-eva-activity-change") {
      if (!applyEvaActivityUndo()) {
        updateStorageStatus("Keine rückgängig zu machende Aktivitätsänderung vorhanden.");
      }
      return;
    }

    if (actionTarget?.dataset.action === "redo-eva-activity-change") {
      if (!applyEvaActivityRedo()) {
        updateStorageStatus("Keine wiederherstellbare Aktivitätsänderung vorhanden.");
      }
      return;
    }

    if (actionTarget?.dataset.action === "set-eva-activity-sort") {
      const sortBy = actionTarget.dataset.sortBy === "name" ? "name" : "newest";
      store.setState((state) => ({
        ...state,
        ui: {
          ...state.ui,
          evaActivitySortBy: sortBy
        }
      }));
      return;
    }
    ...
    if (actionTarget?.dataset.action === "add-mc-variable") {
      store.setState((state) => ({
        ...state,
        monteCarlo: {
          ...state.monteCarlo,
          variables: [...(state.monteCarlo.variables || []), createMonteCarloVariable()]
        }
      }));
      return;
    }

    if (actionTarget?.dataset.action === "remove-mc-variable") {
      const index = Number(actionTarget.dataset.index);
      store.setState((state) => ({
        ...state,
        monteCarlo: {
          ...state.monteCarlo,
          variables: (state.monteCarlo.variables || []).filter((_, itemIndex) => itemIndex !== index)
        }
      }));
      return;
    }
      ...
      store.setState((state) => {
        ...
        return {
          ...
          monteCarlo: {
            ...
            targetLabel: baseline.targetLabel || state.monteCarlo.targetLabel,
            targetUnit: baseline.targetUnit || state.monteCarlo.targetUnit,
            modelName: baseline.modelName || state.monteCarlo.modelName,
            objectiveName: baseline.objectiveName || state.monteCarlo.objectiveName,
            formula: baseline.formula || state.monteCarlo.formula,
            scenarioMode: baseline.scenarioMode || state.monteCarlo.scenarioMode,
            iterations: baseline.iterations,
            seed: baseline.seed,
            measureSettings: JSON.parse(JSON.stringify(baseline.measureSettings || {})),
            variables: JSON.parse(JSON.stringify(baseline.variables || [])),
      ...
      store.setState((state) => {
        ...
        return {
          ...
          monteCarlo: {
            ...
            targetLabel: scenario.targetLabel || state.monteCarlo.targetLabel,
            targetUnit: scenario.targetUnit || state.monteCarlo.targetUnit,
            modelName: scenario.modelName || state.monteCarlo.modelName,
            objectiveName: scenario.objectiveName || state.monteCarlo.objectiveName,
            formula: scenario.formula || state.monteCarlo.formula,
            scenarioMode: scenario.scenarioMode || state.monteCarlo.scenarioMode,
            iterations: scenario.iterations,
            seed: scenario.seed,
            measureSettings: JSON.parse(JSON.stringify(scenario.measureSettings || {})),
            variables: JSON.parse(JSON.stringify(scenario.variables || [])),
    ...
    if (actionTarget?.dataset.action === "show-all-eva-detail-columns") {
      event.preventDefault();
      event.stopPropagation();
      store.setState((state) => ({
        ...state,
  ...
  document.addEventListener("change", (event) => {
    ...
    const mcModelField = event.target.closest("[data-mc-model-field]");
    if (mcModelField) {
      const field = mcModelField.getAttribute("data-mc-model-field");
      const currentValue = store.getState().monteCarlo[field];
      const nextValue = field === "formula" || field === "modelName" || field === "objectiveName" || field === "targetLabel" || field === "targetUnit" || field === "scenarioMode"
        ? String(mcModelField.value)
        : parseValue(currentValue, mcModelField.value);
      store.setState((state) => ({
        ...state,
        monteCarlo: {
          ...state.monteCarlo,
    ...
    const mcVariableField = event.target.closest("[data-mc-variable-field]");
    if (mcVariableField) {
      const index = Number(mcVariableField.getAttribute("data-mc-variable-index"));
      const field = mcVariableField.getAttribute("data-mc-variable-field");
      store.setState((state) => ({
        ...state,
        monteCarlo: {
          ...state.monteCarlo,
          variables: (state.monteCarlo.variables || []).map((item, itemIndex) => {
            if (itemIndex !== index) return item;
            const nextValue = field === "name" || field === "key" || field === "description" || field === "unit" || field === "distribution" || field === "source" || field === "correlationGroup"
              ? String(mcVariableField.value)
              : field === "active"
                ? Boolean(mcVariableField.checked)
                : parseValue(item[field], mcVariableField.value);
            return {
              ...item,
              [field]: nextValue
            };
