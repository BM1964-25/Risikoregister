import { initialState } from "./state.r273.js";
...
function calculateRiskRegisterResult(state) {
  ...
  const activeCount = risks.filter((risk) => ["offen", "in bearbeitung", "beobachtung", "massnahme laeuft", "maßnahme läuft"].includes(String(risk.status || "").toLowerCase())).length;
  ...
  riskRegister: {
    ...
    render(state) {
      ...
      const classifyStatusTone = (status) => {
        const value = String(status || "offen").trim().toLowerCase();
        if (value === "geschlossen") return { key: "closed", label: "Geschlossen" };
        if (value === "maßnahme läuft" || value === "massnahme läuft") return { key: "action", label: "Maßnahme läuft" };
        if (value === "in bearbeitung") return { key: "progress", label: "In Bearbeitung" };
        if (value === "beobachtung") return { key: "watch", label: "Beobachtung" };
      ...
      const statusOptions = [
        ...
        { value: "maßnahme läuft", label: "Maßnahme läuft" },
      ...
      const columnDefs = [
        ...
        { key: "status", label: "Status", render: (risk) => risk.status || "offen" },
      ...
      return `
        <div class="module-shell risk-register-shell">
          <div class="module-title risk-register-title">
            ...
            <article class="kpi-card gold"><div class="kpi-label">Aktive Risiken</div><div class="kpi-value">${result.activeCount}</div><div class="kpi-sub">offen oder in Bearbeitung</div></article>
          ...
          <details class="info-card risk-fold-card ai-workshop-card" open style="margin-top:18px;">
            ...
            <p class="form-note">Hier hilfst du dir bei der Risikoerfassung und bei der Nacharbeit bestehender Risiken. Freitext wird strukturiert, Maßnahmen werden vorgeschlagen und das Rest-Risiko wird fachlich verdichtet.</p>
            <div class="report-export-grid" style="align-items:start;">
              <div class="form-field wide">
                ...
                <textarea id="aiWorkshopFreeText" data-ai-workshop-field="freeText" placeholder="Beschreibe ein Risiko, eine Maßnahme oder einen Sachverhalt, den die KI strukturieren soll.">${escapeHtml(state.ui?.aiWorkshop?.freeText || "")}</textarea>
              </div>
            </div>
            ...
            <p class="form-note">Die KI wertet zuerst den Freitext aus und zeigt dir einen Vorschlag. Danach entscheidest du, ob der Vorschlag als neues Risiko angelegt oder in ein bestehendes Risiko übernommen werden soll.</p>
                        ...
                        <article class="ai-workshop-result-card">
                          ...
                          <div class="ai-workshop-result-card-grid">
                            <div class="ai-workshop-result-card-column">
                              ...
                              <div><span>Eintrittswahrscheinlichkeit (1-5)</span><strong>${escapeHtml(String(item.likelihood ?? "—"))}</strong></div>
                            </div>
                            <div class="ai-workshop-result-card-column">
            ...
            <p class="form-note">Wenn du ein bestehendes Risiko auswählst, aktualisiert die KI dieses Risiko mit Maßnahmen, Rest-Risiko, Verantwortlich und Fällig am. Andernfalls legt sie einen neuen Vorschlag an.</p>
            <div class="ai-workshop-result-actions" style="margin-top:12px;">
              <button class="action-btn primary" type="button" data-action="apply-ai-workshop-risks">Neues Risiko anlegen</button>
              <button class="action-btn" type="button" data-action="apply-ai-workshop-measures">Bestehendes Risiko aktualisieren</button>
              <button class="action-btn" type="button" data-action="clear-ai-workshop-result">Vorschlag verwerfen</button>
            </div>
            <p class="form-note" style="margin-top:8px;">Mit den Buttons übernimmst du die KI-Vorschläge nach Prüfung in das Risikoregister. Für Maßnahmen nutzt du das Zielrisiko oben.</p>
            </div>
          </details>
          <details class="info-card risk-register-card risk-fold-card risk-fold-edit" open>
            ...
            <div class="risk-fold-body">
              <div style="margin-bottom:14px;">
                <button class="action-btn risk-add-btn" type="button" data-action="add-risk-register-item"><span class="risk-add-icon" aria-hidden="true">+</span><span>Risiko hinzufügen</span></button>
              </div>
              <div class="risk-register-toolbar" style="margin-bottom:12px;">
                <div class="risk-register-toolbar-grid" style="grid-template-columns:minmax(220px, 280px);">
                  <div class="form-field">
                    <label for="risk_edit_sort">Sortierung</label>
                    <div class="risk-select-shell">
                      <select id="risk_edit_sort" data-risk-edit-sort-field="sortBy">
                        <option value="newest" ${String(result.ui?.riskRegisterView?.editSortBy || "newest") === "newest" ? "selected" : ""}>Neueste zuerst</option>
                        <option value="id" ${String(result.ui?.riskRegisterView?.editSortBy || "") === "id" ? "selected" : ""}>Risiko-ID aufsteigend</option>
                      </select>
                      <span class="risk-select-arrow" aria-hidden="true">▾</span>
                    </div>
                    <div class="risk-sort-actions" style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
                      <button class="risk-chart-chip ${String(result.ui?.riskRegisterView?.editSortBy || "newest") === "newest" ? "active" : ""}" type="button" data-action="set-risk-register-edit-sort" data-sort-by="newest">Neueste zuerst</button>
                      <button class="risk-chart-chip ${String(result.ui?.riskRegisterView?.editSortBy || "") === "id" ? "active" : ""}" type="button" data-action="set-risk-register-edit-sort" data-sort-by="id">Risiko-ID aufsteigend</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              ${result.editRisks.map((risk, index) => `
                <details class="risk-edit-card risk-tone-${classifyRiskTone(risk).key} risk-fold-card risk-edit-fold" open>
                  ...
                  <div class="risk-edit-grid">
                    ...
                    <div class="form-field risk-select-field">
                    ...
                    <div class="form-field">
                      ...
                      <input id="risk_phase_${index}" data-risk-index="${index}" data-risk-field="phase" type="text" value="${risk.phase}">
                    </div>
                    <div class="form-field wide">
                      <label for="risk_description_${index}">Risikobeschreibung</label>
                      <textarea id="risk_description_${index}" data-risk-index="${index}" data-risk-field="description">${risk.description}</textarea>
                    </div>
                    <div class="form-field">
                      <label for="risk_area_${index}">Risikobereich</label>
                      <input id="risk_area_${index}" data-risk-index="${index}" data-risk-field="area" type="text" value="${risk.area}">
                    </div>
                    <div class="form-field">
                      <label for="risk_owner_${index}">Verantwortlich</label>
                      <input id="risk_owner_${index}" data-risk-index="${index}" data-risk-field="owner" type="text" value="${risk.owner}">
                    </div>
                    <div class="form-field">
                      ...
                      <div class="risk-select-shell">
                        <select id="risk_status_${index}" data-risk-index="${index}" data-risk-field="status">
                          ${statusOptions.map((status) => `
                            <option value="${status.value}" ${String(risk.status || "offen").toLowerCase() === status.value ? "selected" : ""}>${status.label}</option>
                          `).join("")}
                        </select>
                        <span class="risk-select-arrow" aria-hidden="true">▾</span>
                      </div>
                    ...
                    <div class="risk-output-grid wide">
                      <div class="form-field">
                        ...
                        <div class="eva-stepper-field risk-stepper-field">
                          <input class="compact-input integer-input" id="risk_likelihood_${index}" data-risk-index="${index}" data-risk-field="likelihood" type="text" inputmode="numeric" value="${formatIntegerInput(risk.likelihood)}">
                          <div class="eva-stepper-buttons">
                            <button type="button" class="eva-stepper-btn" data-action="risk-step" data-field="likelihood" data-step="1" data-direction="up" data-index="${index}" aria-label="Eintrittswahrscheinlichkeit erhöhen">▲</button>
                            <button type="button" class="eva-stepper-btn" data-action="risk-step" data-field="likelihood" data-step="1" data-direction="down" data-index="${index}" aria-label="Eintrittswahrscheinlichkeit verringern">▼</button>
                          </div>
                        </div>
                      </div>
                      <div class="form-field">
                    ...
                    <div class="risk-residual-stack">
                      <div class="form-field">
                        ...
                        <input id="risk_residual_${index}" data-risk-index="${index}" data-risk-field="residualRisk" type="text" value="${risk.residualRisk}">
                      </div>
