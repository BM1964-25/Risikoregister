import { createStore, cloneState } from "./state.r268.js";
import { modules, riskCategoryOptions, calculatePertResult, calculateMonteCarloResult, calculateEarnedValueResult, buildManagementReportData, buildSelectedReportData, renderSelectedReportText, renderRiskReportText } from "./modules.r268.js";

const store = createStore(cloneState());
const AUTOSAVE_KEY = "project_controls_hub_autosave_v2";
      ...
      items: [
        {
          ...
          category: selectedRisk?.category || riskCategoryOptions[4] || "Projekt- und Managementrisiken",
          phase: selectedRisk?.phase || "",
          owner: selectedRisk?.owner || "",
          probabilityPercent: Number(selectedRisk?.probabilityPercent) || 50,
          likelihood: Number(selectedRisk?.likelihood) || 3,
          impact: Number(selectedRisk?.impact) || 3,
          financialImpact: Number(selectedRisk?.financialImpact) || 0,
          measures: selectedRisk?.measures || "",
          residualRisk: selectedRisk?.residualRisk || ""
        }
      ]
    };
  }
      ...
      items: [
        {
          ...
          residualRisk: selectedRisk?.residualRisk || "Rest-Risiko bitte fachlich prüfen.",
          owner: selectedRisk?.owner || "",
...
function mapSuggestedRiskToRegisterItem(suggestion = {}, fallbackId = "") {
  const likelihood = Math.max(1, Math.min(5, Number(suggestion.likelihood) || Number(suggestion.probability) || Math.ceil((Number(suggestion.probabilityPercent) || 0) / 20) || 3));
  const probabilityPercent = Math.max(
    0,
    Math.min(
      100,
      Number(suggestion.probabilityPercent) || (Number.isFinite(likelihood) ? likelihood * 20 : 0)
    )
  );
  return {
    ...
    category: String(suggestion.category || riskCategoryOptions[4] || "Projekt- und Managementrisiken").trim() || (riskCategoryOptions[4] || "Projekt- und Managementrisiken"),
    area: "",
    financialImpact: Math.max(0, Number(suggestion.financialImpact) || 0),
    probabilityPercent,
    expectedDamage: Math.max(0, Number(suggestion.financialImpact) || 0),
    likelihood,
    ...
    status: "offen",
    residualRisk: String(suggestion.residualRisk || "").trim()
  };
}
  ...
  store.setState((current) => ({
    ...
    riskRegister: {
      ...
      risks: current.riskRegister.risks.map((risk) => risk.id === selectedId ? {
        ...
        residualRisk: String(suggestion.residualRisk || risk.residualRisk || ""),
...
async function runAiWorkshopTask(task) {
  ...
  try {
    ...
    const parsedResult = parseAiWorkshopResult(payload.text, task) || ((task === "free-text-risks" || task === "measures-residual") ? buildAiWorkshopFallbackResult(currentState, task, payload.text) : null);
    setAiWorkshopState({
      ...currentState.ui?.aiWorkshop,
      activeTask: task,
      busy: false,
    ...
    const fallbackResult = (task === "free-text-risks" || task === "measures-residual")
      ? buildAiWorkshopFallbackResult(currentState, task, String(error?.message || ""))
      : null;
    setAiWorkshopState({
      ...currentState.ui?.aiWorkshop,
      activeTask: task,
...
function createRiskRegisterItem() {
  return {
    ...
    description: "",
    phase: "",
    category: riskCategoryOptions[4] || "Projekt- und Managementrisiken",
    area: "",
    financialImpact: 0,
...
function createMonteCarloRiskFromRegister(risk) {
  ...
  return {
    ...
    minDelayDays: Math.max(0, Number(risk.likelihood) || 0),
...
function bindEvents() {
  ...
  document.addEventListener("input", (event) => {
    ...
    if (aiWorkshopField && aiWorkshopField.getAttribute("data-ai-workshop-field") === "freeText") {
    ...
    if (aiWorkshopField && aiWorkshopField.getAttribute("data-ai-workshop-field") === "selectedRiskId") {
      const nextValue = aiWorkshopField.value;
      store.setState((state) => ({
        ...state,
        ui: {
